(ns nubank.main
  "Kotodama WASM entrypoint for the Nubank clean-room actor (L4) — Clojure port.
  ...
  facts (`nubank.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "nubank")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"nubank.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "nubank-compat" :tier tier :entities entities} 200])
...
(defn start! [] :nubank-compat/ready)
